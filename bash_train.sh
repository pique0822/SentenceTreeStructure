...
do
	singularity exec -B /om/group/cpl -B "$PROJECT_PATH" "$SINGULARITY_IMG" python3 gru_arithmetic.py \
		--num_epochs 500 --hidden_size $hidden_value --use_cuda True --training_set datasets/arithmetic/fixed_L4_1e3/training.txt --testing_set datasets/arithmetic/fixed_L4_1e3/testing.txt --model_prefix arithmetic_L4_1e3_fixed\
		|| exit 1
done
